...
| `organizer_public_examples.json` | 10 REAL retired scoring examples straight from the organizers' official self-check doc (received Day 3), covering factual_knowledge, mathematical_reasoning, sentiment_classification, text_summarisation, named_entity_recognition. Far more authoritative than our own invented eval_set.json since these are actual examples the real judge was built around. Run with `--eval organizer_public_examples.json`. Notably: the organizers' own sentiment rubric (accept Mixed/Neutral/Positive, reject Negative, require both sides acknowledged) closely matches how we'd already designed sent-01 -- good validation. Also revealed a summary constraint pattern we didn't handle yet (exact bullet count + per-bullet word limit), now added to score_summary(). No logical_reasoning/code_debugging/code_generation examples were included in the public set -- doesn't mean they're less important, the guide still lists all 8 categories for the hidden set. |
